...
from app.services.storage.s3 import presign_put
from app.services.storage.scan import precheck_filename
...
def presign_upload(
    req: PresignRequest, current_user: Annotated[User, Depends(get_current_user)]
):
    precheck_filename(req.filename)
    url = presign_put(key=req.object_key, content_type=req.content_type)
    return {'method': 'PUT', 'url': url, 'headers': {'Content-Type': req.content_type}}
