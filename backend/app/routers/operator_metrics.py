...
def metrics_summary() -> Dict[str, Any]:
    ...
    try:
        ...
        for metric in REGISTRY.collect():
            ...
            if name == "web_automation_tasks_started":
                ...
                for s in metric.samples:
                    status = s.labels.get("status") if s.labels else "unknown"
                    buckets[status] = buckets.get(status, 0) + int(s.value or 0)
                summary["tasks_completed"] = buckets
            elif name == "web_automation_actions_total":
                # Counter with label action
                buckets: Dict[str, int] = {}
                for s in metric.samples:
                    action = s.labels.get("action") if s.labels else "unknown"
                    buckets[action] = buckets.get(action, 0) + int(s.value or 0)
                summary["actions"] = buckets
            elif name == "web_automation_action_errors_total":
                buckets: Dict[str, int] = {}
                for s in metric.samples:
                    action = s.labels.get("action") if s.labels else "unknown"
                    buckets[action] = buckets.get(action, 0) + int(s.value or 0)
                summary["action_errors"] = buckets
                ...
                for s in metric.samples:
                    if s.name.endswith("_bucket"):
                        try:
                            ...
                            buckets.append((le, float(s.value or 0.0)))
                        except Exception:
                            continue
    ...
    try:
        s = get_settings()
        if s.GRAFANA_BASE_URL and s.GRAFANA_DASHBOARD_UID:
            summary["grafana"] = {
                "base_url": s.GRAFANA_BASE_URL,
                "dashboard_uid": s.GRAFANA_DASHBOARD_UID,
                "dashboard_url": f"{s.GRAFANA_BASE_URL}/d/{s.GRAFANA_DASHBOARD_UID}",
            }
    except Exception:
        pass
    return summary
