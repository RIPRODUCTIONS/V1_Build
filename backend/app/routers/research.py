...
# Initialize event bus (Redis if available, in-memory fallback)
event_bus = get_bus()

# Module-level dependency variables to avoid B008 warnings
get_db_dep = Depends(get_db)
require_research_read_dep = Depends(require_scope_hs256('research.read'))
require_research_run_dep = Depends(require_scope_hs256('research.run'))


@router.post('/market-gaps/run')
async def run_market_gap_scanner(
    db: Session = get_db_dep, subject: str = require_research_read_dep
) -> dict[str, Any]:
    ...
    try:
        ...
        event_bus.emit('automation.run.requested', event)
...
async def validate_idea(
    ...
    subject: str = require_research_run_dep,
) -> dict[str, Any]:
...
async def get_validations(
    ...
    limit: int = 20,
    db: Session = get_db_dep,
    subject: str = require_research_read_dep,
) -> dict[str, Any]:
